...
void matmul3(size_t r1, size_t c, size_t c2, float *m1, float *m2, float *result)
{
    ...
    for (size_t i = 0; i < r1; i ++) //i+=8
	{
		for (size_t j = 0; j < c2; j+=8) //j++
		{
            __m256 c0 = _mm256_load_ps(result+i*c2+j); /* c0 = C[i][j] *///result+i+j*r1
			for (size_t k = 0; k < c; k++) 
			{
				c0 = _mm256_add_ps(c0,
					_mm256_mul_ps(_mm256_load_ps(m1+i*c+k),//m1+i+k*r1
						_mm256_broadcast_ss(m2+k*c2+j)));//m2+k+j*c
			}
			_mm256_store_ps(result+i*c2+j, c0);  /* C[i][j] = c0 */;
		}
	}
}
...
{
	for (size_t i = 0; i < r1; i ++) //+= 8*UNROLL
    {
